...
class CrossValidation():
    ...
    def HalvingRandom_search_cv(self) -> None:
        ...
        model = XGBClassifier(
            ...
            eval_metric="logloss",
            tree_method="hist",             # <=== USA GPU PARA ENTRENAMIENTO
            device="cuda",                  # <=== USA GPU PARA PREDICCIÓN (opcional)
            random_state=42,
            n_jobs=-1
        )
...
class XGBoostModel():
    ...
    def training(self,
        ...
        print("Booster type:", self.xgb_model.get_booster().attributes().get('predictor'))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning_rate", type=float, default=0.22667577051792,
                        help="Step size shrinkage used in update to prevent overfitting")
    parser.add_argument("--subsample", type=float, default=0.7827541736614870,
                        help="Subsample ratio of the training instances (row sampling)")
    parser.add_argument("--colsample_bytree", type=float, default=0.5749587136743869,
                        help="Subsample ratio of columns when constructing each tree (feature sampling)")
    parser.add_argument("--gamma", type=float, default=1.1462569761632073,
                        help="Minimum loss reduction required to make a further partition on a leaf node")
    parser.add_argument("--reg_alpha", type=float, default=0.012203072318044517,
                        help="L1 regularization term on weights (encourages sparsity)")
    parser.add_argument("--reg_lambda", type=float, default=0.598442701205227,
    ...
    parser.add_argument("--best_features_rate", type=float, default=0.9,
